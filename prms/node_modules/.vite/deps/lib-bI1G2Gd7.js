import { $ as createRelativePositionFromTypeIndex, $t as toBase64, C as UndoManager, Dt as isDeleted, F as YXmlElement, J as createDeleteSet, N as YText, Ot as isParentOf, Qt as encodeAny, Ut as snapshot, V as applyUpdateV2, Xt as timeout, Z as createID, Zt as doc, _ as Item, an as unexpectedCase, bt as findIndexSS, cn as min, dn as setIfUndefined, en as isBrowser, et as createSnapshot, f as ContentString, in as methodUnimplemented, kt as iterateDeletedStructs, ln as create, m as Doc, nn as keys, on as BIT30, q as createAbsolutePositionFromRelativePosition, qt as typeListToArraySnapshot, rn as oneOf, sn as max, tn as every, u as ContentFormat, un as create$1, x as Snapshot, xt as findRootTypeKey, y as RelativePosition, z as YXmlText } from "./yjs-41sm-D5W.js";
import { Gt as NodeSelection, Kt as Plugin, Wt as AllSelection, Xt as TextSelection, en as Fragment, qt as PluginKey, tn as Slice } from "./dist-SfxboS_3.js";
...
//#endregion
//#region node_modules/y-prosemirror/src/utils.js
/**
...
var restoreRelativeSelection = (tr, relSel, binding) => {
	if (relSel !== null && relSel.anchor !== null && relSel.head !== null) if (relSel.type === "all") tr.setSelection(new AllSelection(tr.doc));
...
var createNodeIfNotExists = (el, schema, meta, snapshot, prevSnapshot, computeYChange) => {
	...
	if (node === void 0) if (el instanceof YXmlElement) return createNodeFromYElement(el, schema, meta, snapshot, prevSnapshot, computeYChange);
...
var absolutePositionToRelativePosition = (pos, type, mapping) => {
	if (pos === 0) return createRelativePositionFromTypeIndex(type, 0, type.length === 0 ? -1 : 0);
	/**
	* @type {any}
	*/
	...
	while (n !== null && type !== n) {
		if (n instanceof YXmlText) {
			if (n._length >= pos) return createRelativePositionFromTypeIndex(n, pos, type.length === 0 ? -1 : 0);
			else pos -= n._length;
...
function yXmlFragmentToProsemirrorJSON(xmlFragment) {
	...
	*/
	const serialize = (item) => {
		...
		*/
		let response;
		if (item instanceof YXmlText) response = item.toDelta().map(
			/** @param {any} d */
			(d) => {
				const text = {
					type: "text",
				...
				if (d.attributes) text.marks = Object.keys(d.attributes).map((type_) => {
					const attrs = d.attributes[type_];
					const mark = { type: yattr2markname(type_) };
					if (Object.keys(attrs)) mark.attrs = attrs;
					return mark;
				});
				return text;
			}
			...
			response = { type: item.nodeName };
			const attrs = item.getAttributes();
			if (Object.keys(attrs).length) response.attrs = attrs;
			const children = item.toArray();
			if (children.length) response.content = children.map(serialize).flat();
		} else unexpectedCase();
		return response;
	};
	return {
...
//#endregion
export { getRelativeSelection as a, ySyncPluginKey as c, yXmlFragmentToProsemirrorJSON as i, yUndoPluginKey as l, relativePositionToAbsolutePosition as n, ySyncPlugin as o, setMeta as r, yCursorPluginKey as s, absolutePositionToRelativePosition as t };

//# sourceMappingURL=lib-bI1G2Gd7.js.map