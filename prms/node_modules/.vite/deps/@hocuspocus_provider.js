import { B as applyUpdate, O as YArray, ht as encodeStateVector, j as YMap, m as Doc, pt as encodeStateAsUpdate, v as PermanentUserData } from "./yjs-41sm-D5W.js";
//#region node_modules/@hocuspocus/common/dist/hocuspocus-common.esm.js
/**
...
/* c8 ignore next */
var utf8TextEncoder$1 = typeof TextEncoder !== "undefined" ? new TextEncoder() : null;
/**
* @param {string} str
* @return {Uint8Array}
*/
var _encodeUtf8Native$1 = (str) => utf8TextEncoder$1.encode(str);
/**
* @param {string} str
* @return {Uint8Array}
*/
/* c8 ignore next */
var encodeUtf8$1 = utf8TextEncoder$1 ? _encodeUtf8Native$1 : _encodeUtf8Polyfill$1;
/* c8 ignore next */
var utf8TextDecoder$1 = typeof TextDecoder === "undefined" ? null : new TextDecoder("utf-8", {
	fatal: true,
	ignoreBOM: true
});
/* c8 ignore start */
...
*/
var writeVarUint$1 = (encoder, num) => {
	while (num > BITS7$1) {
...
*/
var _strBuffer$1 = new Uint8Array(3e4);
var _maxStrBSize$1 = _strBuffer$1.length / 3;
/**
...
*/
var _writeVarStringNative$1 = (encoder, str) => {
	if (str.length < _maxStrBSize$1) {
		...
		for (let i = 0; i < written; i++) write$1(encoder, _strBuffer$1[i]);
	} else writeVarUint8Array$1(encoder, encodeUtf8$1(str));
};
/**
* Write a variable length string.
*
...
* @param {String} str The string that is to be encoded.
*/
var _writeVarStringPolyfill$1 = (encoder, str) => {
	const encodedString = unescape(encodeURIComponent(str));
	const len = encodedString.length;
...
*/
/* c8 ignore next */
var writeVarString$1 = utf8TextEncoder$1 && utf8TextEncoder$1.encodeInto ? _writeVarStringNative$1 : _writeVarStringPolyfill$1;
/**
...
*/
var writeUint8Array$1 = (encoder, uint8Array) => {
	const bufferLen = encoder.cbuf.length;
...
* @param {Uint8Array} uint8Array
*/
var writeVarUint8Array$1 = (encoder, uint8Array) => {
...
* @module decoding
*/
var errorUnexpectedEndOfArray$1 = create$3("Unexpected end of array");
var errorIntegerOutOfRange$1 = create$3("Integer out of Range");
/**
...
* @return {Uint8Array}
*/
var readUint8Array$1 = (decoder, len) => {
...
* @return {Uint8Array}
*/
var readVarUint8Array$1 = (decoder) => readUint8Array$1(decoder, readVarUint$1(decoder));
/**
...
*/
var readUint8$1 = (decoder) => decoder.arr[decoder.pos++];
/**
...
var readVarUint$1 = (decoder) => {
	...
	while (decoder.pos < len) {
		...
		mult *= 128;
		if (r < BIT8$1) return num;
		/* c8 ignore start */
		if (num > MAX_SAFE_INTEGER$1) throw errorIntegerOutOfRange$1;
	}
...
*/
/* c8 ignore start */
var _readVarStringPolyfill$1 = (decoder) => {
	let remainingLen = readVarUint$1(decoder);
	if (remainingLen === 0) return "";
	else {
		let encodedString = String.fromCodePoint(readUint8$1(decoder));
		if (--remainingLen < 100) while (remainingLen--) encodedString += String.fromCodePoint(readUint8$1(decoder));
...
* @return {String} The read String
*/
var _readVarStringNative$1 = (decoder) => utf8TextDecoder$1.decode(readVarUint8Array$1(decoder));
/**
...
*/
/* c8 ignore next */
var readVarString$1 = utf8TextDecoder$1 ? _readVarStringNative$1 : _readVarStringPolyfill$1;
var AuthMessageType;
...
//#endregion
//#region node_modules/@lifeomic/attempt/dist/es6/src/index.js
...
function defaultCalculateDelay(context, options) {
	...
	if (options.factor) {
		...
		if (options.maxDelay !== 0) delay = Math.min(delay, options.maxDelay);
	}
	if (options.jitter) {
		const min = Math.ceil(options.minDelay);
		delay = Math.floor(Math.random() * (Math.floor(delay) - min + 1)) + min;
	}
...
async function retry(attemptFunc, attemptOptions) {
	...
	for (const prop of [
		...
		if (!Number.isInteger(value) || value < 0) throw new Error(`Value for ${prop} must be an integer greater than or equal to 0`);
	}
	if (options.factor.constructor !== Number || options.factor < 0) throw new Error(`Value for factor must be a number greater than or equal to 0`);
	if (options.delay < options.minDelay) throw new Error(`delay cannot be less than minDelay (delay: ${options.delay}, minDelay: ${options.minDelay}`);
	const context = {
		attemptNum: 0,
	...
	async function makeAttempt() {
		if (options.beforeAttempt) options.beforeAttempt(context, options);
		if (context.aborted) {
			const err = /* @__PURE__ */ new Error(`Attempt aborted`);
			err.code = "ATTEMPT_ABORTED";
			throw err;
		}
		const onError = async (err) => {
			if (options.handleError) await options.handleError(err, context, options);
			if (context.aborted || context.attemptsRemaining === 0) throw err;
			context.attemptNum++;
			const delay = calculateDelay(context, options);
			if (delay) await sleep(delay);
			return makeAttempt();
		};
		if (context.attemptsRemaining > 0) context.attemptsRemaining--;
		if (options.timeout) return new Promise((resolve, reject) => {
			const timer = setTimeout(() => {
				if (options.handleTimeout) try {
					resolve(options.handleTimeout(context, options));
				} catch (e) {
					reject(e);
				}
				else {
					const err = /* @__PURE__ */ new Error(`Retry timeout (attemptNum: ${context.attemptNum}, timeout: ${options.timeout})`);
		...
		else return attemptFunc(context, options).catch(onError);
	}
...
//#endregion
//#region node_modules/@hocuspocus/provider/dist/hocuspocus-provider.esm.js
/**
...
var HocuspocusProviderWebsocket = class extends EventEmitter {
	constructor(configuration) {
		...
		if (typeof configuration.connect !== "undefined") this.shouldConnect = configuration.connect;
	...
	attach(provider) {
		...
		if (this.status === WebSocketStatus.Disconnected && this.shouldConnect) connectPromise = this.connect();
		if (this.receivedOnOpenPayload) provider.onOpen(this.receivedOnOpenPayload);
		if (this.receivedOnStatusPayload) provider.onStatus(this.receivedOnStatusPayload);
	...
	cleanupWebSocket() {
		...
		Object.keys(handlers).forEach((name) => {
			var _a;
			(_a = this.webSocket) === null || _a === void 0 || _a.removeEventListener(name, handlers[name]);
	...
	onMessage(event) {
		...
		(_a = this.configuration.providerMap.get(documentName)) === null || _a === void 0 || _a.onMessage(event);
	}
	...
	rejectConnectionAttempt() {
		var _a;
		(_a = this.connectionAttempt) === null || _a === void 0 || _a.reject();
		this.connectionAttempt = null;
	}
	checkConnection() {
		var _a;
		...
		if (this.closeTries > 2) {
			...
			(_a = this.webSocket) === null || _a === void 0 || _a.close();
			this.messageQueue = [];
		}
	}
	get serverUrl() {
		while (this.configuration.url[this.configuration.url.length - 1] === "/") return this.configuration.url.slice(0, this.configuration.url.length - 1);
	...
	send(message) {
		var _a;
		if (((_a = this.webSocket) === null || _a === void 0 ? void 0 : _a.readyState) === WsReadyStates.Open) this.webSocket.send(message);
	...
	onClose({ event }) {
		...
		if (event.code === Unauthorized.code) {
			if (event.reason === Unauthorized.reason) console.warn("[HocuspocusProvider] An authentication token is required, but you didn’t send one. Try adding a `token` to your HocuspocusProvider configuration. Won’t try again.");
			else console.warn(`[HocuspocusProvider] Connection closed with status Unauthorized: ${event.reason}`);
			this.shouldConnect = false;
		}
		...
		else if (this.shouldConnect) this.connect();
		if (this.shouldConnect) return;
		if (this.status === WebSocketStatus.Disconnected) return;
		this.status = WebSocketStatus.Disconnected;
		this.emit("status", { status: WebSocketStatus.Disconnected });
		this.emit("disconnect", { event });
	}
...
var MessageReceiver = class {
	...
	apply(provider, emitSynced) {
		...
		if (message.length() > emptyMessageLength + 1) if (this.broadcasted) provider.broadcast(OutgoingMessage, { encoder: message.encoder });
		else provider.send(OutgoingMessage, { encoder: message.encoder });
	}
...
var MessageSender = class {
	...
	send(webSocket) {
		webSocket === null || webSocket === void 0 || webSocket.send(this.create());
	}
	broadcast(channel) {
...
var SyncStepTwoMessage = class extends OutgoingMessage {
	...
	get(args) {
		if (typeof args.document === "undefined") throw new Error("The sync step two message requires document as an argument");
		writeVarString(this.encoder, args.documentName);
		writeVarUint(this.encoder, this.type);
...
var HocuspocusProvider = class extends EventEmitter {
	constructor(configuration) {
		...
		(_a = this.awareness) === null || _a === void 0 || _a.on("update", () => {
			this.emit("awarenessUpdate", { states: awarenessStatesToArray(this.awareness.getStates()) });
		});
		(_b = this.awareness) === null || _b === void 0 || _b.on("change", () => {
			this.emit("awarenessChange", { states: awarenessStatesToArray(this.awareness.getStates()) });
		});
		this.document.on("update", this.boundDocumentUpdateHandler);
		(_c = this.awareness) === null || _c === void 0 || _c.on("update", this.boundAwarenessUpdateHandler);
	...
	*/
	...
	async connect() {
		if (this.configuration.broadcast) this.subscribeToBroadcastChannel();
	...
	disconnect() {
		...
		if (!this.configuration.preserveConnection) this.configuration.websocketProvider.disconnect();
	}
	async onOpen(event) {
		this.isAuthenticated = false;
		this.isConnected = true;
		...
		if (this.isAuthenticationRequired) this.send(AuthenticationMessage, {
			token,
			documentName: this.configuration.name
		});
		this.startSync();
	...
	subscribeToBroadcastChannel() {
		...
		this.mux(() => {
			...
			if (this.awareness) this.broadcast(AwarenessMessage, {
				awareness: this.awareness,
				clients: [this.document.clientID],
				document: this.document,
				documentName: this.configuration.name
	...
	disconnectBroadcastChannel() {
		if (this.awareness) this.send(AwarenessMessage, {
			awareness: this.awareness,
			clients: [this.document.clientID],
	...
	broadcast(Message, args) {
		if (!this.configuration.broadcast) return;
		if (!this.subscribedToBroadcastChannel) return;
...
var defaultGetThreadsOptions = { types: ["unarchived"] };
...
var TiptapCollabProvider = class extends HocuspocusProvider {
	constructor(configuration) {
		if (!configuration.websocketProvider) configuration.websocketProvider = new TiptapCollabProviderWebsocket({
			appId: configuration.appId,
			baseUrl: configuration.baseUrl
		});
		if (!configuration.token) configuration.token = "notoken";
		super(configuration);
	...
	*/
	getThreads(options) {
		const { types } = {
			...defaultGetThreadsOptions,
			...options
		};
		const threads = this.getYThreads().toJSON();
		if ((types === null || types === void 0 ? void 0 : types.includes("archived")) && (types === null || types === void 0 ? void 0 : types.includes("unarchived"))) return threads;
		return threads.filter((currentThead) => {
			if ((types === null || types === void 0 ? void 0 : types.includes("archived")) && currentThead.deletedAt) return true;
			if ((types === null || types === void 0 ? void 0 : types.includes("unarchived")) && !currentThead.deletedAt) return true;
			return false;
		});
	}
	/**
	...
	*/
	updateThread(id, data) {
		...
		this.document.transact(() => {
			...
			if (data.resolvedAt || data.resolvedAt === null) thread.set("resolvedAt", data.resolvedAt);
	...
	*/
	getThreadComments(threadId, includeDeleted) {
		var _a, _b, _c;
		if (this.getThreadIndex(threadId) === null) return null;
	...
	*/
	updateComment(threadId, commentId, data) {
		...
		this.document.transact(() => {
			...
			if (thread === null) return null;
			let comment = null;
			for (const c of thread.get("comments")) if (c.get("id") === commentId) {
				comment = c;
				break;
			}
	...
	*/
	deleteComment(threadId, commentId, options) {
		const { deleteContent, deleteThread } = {
			...defaultDeleteCommentOptions,
			...options
		};
		const thread = this.getYThread(threadId);
		if (thread === null) return null;
		...
		if (!thread.get("deletedComments")) thread.set("deletedComments", new YArray());
