        ...
        foreach ($stages as $stage) {
            foreach ($stage->date_reminders_json as $reminder) {
                ...
                foreach ($records as $record) {
                    ...
                    $message = "This is a reminder that \"{$title}\" is scheduled for "
                        . Carbon::parse($targetDate)->format('F d, Y')
                        . " ({$daysBefore} day(s) from today).";

                    try {
                        $this->notifications->notifyRecipients($recipients, $record, $message, $subject);
                        $sent += count($recipients);
                    } catch (\Throwable $e) {
                        Log::warning('[date-field-reminders] record=' . $record->id . ': ' . $e->getMessage());
                    }
